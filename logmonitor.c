...
int main(int argc, char **argv)
{
    ...
    if (IS_SUCCESS(retval)) {
        for (int i = optind; i < argc; ++i) {
            ...
            if (ctx.num_monitored_files == DIM(ctx.monitored_files)) {
                SET_ERROR(retval, "Too many files to monitor.");
            }
            ...
            if (fpath[0] == 's' && fpath[1] == ':') {
                fpath += 2;
                ctx.monitored_files[ctx.num_monitored_files].is_status = true;
            }
            else {
                ctx.monitored_files[ctx.num_monitored_files].is_status = false;
            }

            ctx.monitored_files[ctx.num_monitored_files].path = fpath;
    ...
    while (IS_SUCCESS(retval)) {
        ...
        do {
            ...
            for (;;) {
                ...
                //write(STDOUT_FILENO, tailbuf, nread);
            }
